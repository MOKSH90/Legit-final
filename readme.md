...
Legit is a secure document verification platform built around one core idea: documents stay inside the vault, while service providers receive only verification results.

Instead of sending Aadhaar, PAN, certificates, or other documents to every company that asks for them, Legit lets a requester create a verification contract. The user reviews what is being requested, approves or rejects it, and the backend performs verification internally. The requester receives a PASS, FAIL, or PARTIAL result with proof metadata, not the original document data.
