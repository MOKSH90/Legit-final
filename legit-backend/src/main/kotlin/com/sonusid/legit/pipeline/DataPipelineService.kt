...
class DataPipelineService(
    ...
    suspend fun createContract(
        ...
        if (request.userId != null) {
            ...
            // Try resolving by legitId/dedoxId first (username.suffix@dedox or @legit)
            val isDedoxHandle = identifier.contains("@dedox") || identifier.contains("@legit")
            val userByLegitId = if (isDedoxHandle) {
                userService.getUserByLegitId(identifier)
            } else null

            if (userByLegitId != null) {
                logger.info("User found by Dedox ID: {} -> Internal ID: {}", identifier, userByLegitId.id)
            }

            // Fallback to internal ID if not a dedoxId or not found
            val targetUser = userByLegitId ?: if (!isDedoxHandle) {
    ...
    suspend fun approveContract(userId: String, request: ContractApprovalRequest): Result<Any> {
        ...
        // Automatically promote officer's evidence documents to UNDER_ANALYSIS upon confirmation
        try {
            MongoDB.documents.updateMany(
                Filters.and(
                    Filters.eq("userId", userId),
                    Filters.`in`("status", listOf(DocumentStatus.SUBMITTED.name, DocumentStatus.ASSIGNED_TO_FORENSICS.name, DocumentStatus.PENDING.name))
                ),
                Updates.combine(
                    Updates.set("status", DocumentStatus.UNDER_ANALYSIS.name),
                    Updates.set("updatedAt", System.currentTimeMillis())
                )
            )
        } catch (e: Exception) {
            logger.warn("Could not automatically update document status on contract approval: {}", e.message)
        }
