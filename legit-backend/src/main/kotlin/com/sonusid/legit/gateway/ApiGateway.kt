...
class ApiGateway(
    ...
    companion object {
        ...
        const val API_NAME = "The Dedox — Secure Data Verification Pipeline"
    }
                ...
                call.respond(
                    ...
                    ApiResponse.success(
                        mapOf(
                            "name" to API_NAME,
                            "version" to VERSION,
                            "message" to "Welcome to The Dedox. Your documents never leave. Only verification does.",
                            "health" to "/api/v1/gateway/health",
                            "info" to "/api/v1/gateway/info",
                            "docs" to "/api/v1/gateway/endpoints"
                        ),
                        "The Dedox API Gateway is running"
                    )
                )
            }
                ...
                get("/info") {
                    val info = GatewayInfo(
                        ...
                        description = buildString {
                            append("The Dedox is a secure data verification pipeline. ")
                            append("Unlike DigiLocker which shares actual documents, The Dedox uses a contractual verification model ")
                    ...
                    call.respond(
                        HttpStatusCode.OK,
                        ApiResponse.success(info, "The Dedox API Gateway Information")
                    )
                }
        ...
        EndpointGroup(
            ...
            endpoints = listOf(
                EndpointInfo(
                    ...
                    description = "Detailed explanation of The Dedox verification pipeline flow",
                    auth = "None"
                ),
                EndpointInfo(
                    method = "GET",
