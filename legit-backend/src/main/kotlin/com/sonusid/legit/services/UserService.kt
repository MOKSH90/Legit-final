...
class UserService(
    ...
    suspend fun register(request: RegisterRequest): Result<AuthResponse> {
        return registerWithRole(request, request.role)
    }

    suspend fun registerServiceProvider(request: RegisterRequest): Result<AuthResponse> {
        return registerWithRole(request, UserRole.SERVICE_PROVIDER)
    }

    private suspend fun registerWithRole(request: RegisterRequest, role: UserRole): Result<AuthResponse> {
        ...
        val user = User(
            username = username,
            legitId = "$username.$suffix@dedox",
            email = normalizedEmail,
            passwordHash = passwordHash,
            fullName = request.fullName.trim(),
            phoneNumber = request.phoneNumber?.trim(),
            isVerified = role == UserRole.SERVICE_PROVIDER || role == UserRole.POLICE_OFFICER || role == UserRole.FORENSICS_SPECIALIST,
            role = role,
        ...
        val finalLegitId = if (user.legitId.isBlank()) {
            val generated = "${user.username}.legacy@dedox"
    ...
    suspend fun refreshToken(refreshToken: String): Result<AuthResponse> {
        ...
        return Result.success(
            AuthResponse(
                ...
                userId = userIdStr,
                username = user.username,
                legitId = if (user.legitId.isBlank()) "${user.username}.legacy@dedox" else user.legitId,
                role = user.role,
                expiresIn = jwtExpirationMs / 1000
            )
        )
    ...
    suspend fun getUserByLegitId(legitId: String): User? {
        ...
        // 2. If not found and it ends with @dedox or @legit, try extracting username
        val suffix = when {
            identifier.endsWith("@dedox") -> "@dedox"
            identifier.endsWith("@legit") -> "@legit"
            else -> null
        }
        if (suffix != null) {
            // Remove suffix
            val prefix = identifier.removeSuffix(suffix)
            logger.info("getUserByLegitId: Prefix extracted: {}", prefix)
            
            // Try 1: The whole prefix is the username (e.g. "john.doe@dedox")
            ...
            // Try 2: Prefix contains a suffix (e.g. "frutus.x92b@dedox" or "john.doe.legacy@dedox")
            if (prefix.contains(".")) {
    ...
    suspend fun updatePublicKey(userId: String, publicKey: String): Result<Unit> {
        return try {
            val result = MongoDB.users.updateOne(
                Filters.eq("_id", ObjectId(userId)),
                Updates.combine(
                    Updates.set("publicKey", publicKey.trim()),
                    Updates.set("updatedAt", System.currentTimeMillis())
                )
            )

            if (result.matchedCount == 0L) {
                Result.failure(NotFoundException(code = ErrorCodes.USER_NOT_FOUND, message = "User not found"))
            } else {
                Result.success(Unit)
            }
        } catch (e: Exception) {
            logger.error("Failed to update public key for user $userId", e)
            Result.failure(e)
        }
    }

    suspend fun getUserPublicKey(userId: String): Result<UserPublicKeyResponse> {
        val user = getUserById(userId) ?: return Result.failure(
    ...
    suspend fun getUsersByRole(role: UserRole): List<UserPublicKeyResponse> {
        return MongoDB.users.find(Filters.eq("role", role.name)).toList().map { user ->
            UserPublicKeyResponse(
                userId = user.id?.toHexString() ?: "",
                username = user.username,
                role = user.role,
    ...
    suspend fun seedDefaultAccounts() {
        val defaultAccounts = listOf(
            Triple("forensics@cfsl.gov.in", "Forensics@123", Triple("cfsl_forensics", "Dr. Ananya Ray (Senior Forensic Examiner)", UserRole.FORENSICS_SPECIALIST)),
            Triple("officer@police.gov.in", "Officer@123", Triple("inspector_vikram", "Inspector Vikramaditya Singh", UserRole.POLICE_OFFICER)),
            Triple("admin@cfsl.gov.in", "Admin@123", Triple("cfsl_director", "Director CFSL (Govt of India)", UserRole.ADMIN)),
            Triple("court@judiciary.gov.in", "Court@123", Triple("sessions_court", "District & Sessions Court Bench", UserRole.LEGAL_AUTHORITY))
        )

        for ((email, password, userDetails) in defaultAccounts) {
            val (username, fullName, role) = userDetails
            val existing = getUserByEmail(email.lowercase().trim())
            if (existing == null) {
                val passwordHash = hashPassword(password)
                val user = User(
                    username = username,
                    legitId = "$username.001@dedox",
                ...
                logger.info("Seeded default account: $email with role $role")
            }
        }
    }
}
