...
class DocumentService(
    ...
    suspend fun uploadDocument(userId: String, request: DocumentUploadRequest): Result<DocumentResponse> {
        ...
        val caseNumber = request.caseNumber?.trim()?.ifBlank { null } ?: request.metadata.caseNumber?.trim()?.ifBlank { null }
        val targetDept = request.targetDepartment?.trim()?.ifBlank { null } ?: request.metadata.targetDepartment?.trim()?.ifBlank { null }
        val targetRecipient = request.targetRecipientId?.trim()?.ifBlank { null } ?: request.metadata.targetRecipientId?.trim()?.ifBlank { null }

        // Check if this exact document already exists for this user (unless it's an evidence record with unique case/doc numbers)
        ...
        if (existing.isNotEmpty()) {
            return Result.failure(
                ConflictException(
                    ...
                    message = "A ${request.documentType.name} with this reference number is already stored"
                )
            )
        }
        ...
        // If client already encrypted the payload with AES-256 (Zero Knowledge), store directly.
        // Otherwise encrypt on server.
        val encryptedData = if (request.metadata.clientEncrypted) {
            request.rawData
        } else {
            encrypt(request.rawData)
        }

        val defaultStatus = when {
            targetDept != null || targetRecipient != null -> DocumentStatus.ASSIGNED_TO_FORENSICS
            isLawEnforcementDoc(request.documentType) -> DocumentStatus.SUBMITTED
        ...
        val document = Document(
            ...
            targetDepartment = targetDept,
            targetRecipientId = targetRecipient,
            issuedBy = request.issuedBy?.trim() ?: request.metadata.officerName?.trim(),
            issuedAt = request.issuedAt ?: System.currentTimeMillis(),
    ...
    suspend fun getDocumentsByCaseNumber(caseNumber: String): List<DocumentSummary> {
        return MongoDB.documents.find(
            Filters.or(
                Filters.eq("caseNumber", caseNumber),
                Filters.eq("metadata.caseNumber", caseNumber)
            )
        ).toList().map { it.toSummary() }
    }

    suspend fun getDocumentsByDepartment(department: String): List<DocumentSummary> {
        return MongoDB.documents.find(
            Filters.or(
                Filters.eq("targetDepartment", department),
                Filters.eq("metadata.targetDepartment", department)
            )
        ).toList().map { it.toSummary() }
    }

    suspend fun getAllEvidenceDocuments(): List<DocumentSummary> {
        return MongoDB.documents.find().toList().map { it.toSummary() }
    }

    suspend fun getDocumentDownload(
        userId: String,
        documentId: String,
        ...
        val isOwner = document.userId == userId
        val isRecipient = document.targetRecipientId == userId
        val isDeptAssigned = !document.targetDepartment.isNullOrBlank() || !document.metadata.targetDepartment.isNullOrBlank()
        val isPrivileged = userRole == UserRole.ADMIN ||
                userRole == UserRole.FORENSICS_SPECIALIST ||
                userRole == UserRole.POLICE_OFFICER ||
                userRole == UserRole.LEGAL_AUTHORITY

        if (!isOwner && !isRecipient && !isDeptAssigned && !isPrivileged) {
            return Result.failure(
        ...
        val integrityResult = verifyDocumentIntegrity(documentId).getOrDefault(false)
        ...
        return Result.success(
            DocumentDownloadResponse(
                ...
                caseNumber = document.caseNumber ?: document.metadata.caseNumber,
                dataHash = document.dataHash,
                encryptedData = document.encryptedData,
                clientEncrypted = document.metadata.clientEncrypted,
                clientKeyWrap = document.metadata.clientKeyWrap,
                metadata = document.metadata,
                verifiedIntegrity = integrityResult
            )
        )
    }
    ...
    suspend fun getDocumentResponse(userId: String, documentId: String, userRole: UserRole? = null): Result<DocumentResponse> {
        val document = getDocumentById(documentId)
            ?: return Result.failure(
                NotFoundException(
                    code = ErrorCodes.DOCUMENT_NOT_FOUND,
        ...
        // Ensure the requesting user owns this document or is authorized
        if (!isOwner && !isRecipient && !isDeptAssigned && !isPrivileged) {
            return Result.failure(
                AuthorizationException(
    ...
    suspend fun updateDocumentStatus(
        ...
        val isOwner = userId != null && document.userId == userId
        val isRecipient = userId != null && document.targetRecipientId == userId
        val isDeptAssigned = !document.targetDepartment.isNullOrBlank() || !document.metadata.targetDepartment.isNullOrBlank()
        val isPrivileged = userRole == UserRole.ADMIN ||
                userRole == UserRole.FORENSICS_SPECIALIST ||
                userRole == UserRole.POLICE_OFFICER ||
        ...
        // If userId is provided, make sure it matches (ownership check, recipient check, or department/privileged check)
        if (userId != null && !isOwner && !isRecipient && !isDeptAssigned && !isPrivileged) {
            return Result.failure(
                AuthorizationException(
    ...
    /**
     * Verifies document integrity by comparing the stored hash with a fresh hash of the data.
     */
    suspend fun verifyDocumentIntegrity(documentId: String): Result<Boolean> {
        ...
        return try {
            if (document.metadata.clientEncrypted) {
                val currentHash = sha256Hash(document.encryptedData)
    ...
    suspend fun hasRequiredDocuments(userId: String, requiredTypes: List<DocumentType>): Pair<Boolean, List<DocumentType>> {
        if (requiredTypes.isEmpty()) return Pair(true, emptyList())

        val validStatuses = listOf(
            DocumentStatus.PENDING.name,
            DocumentStatus.SUBMITTED.name,
            DocumentStatus.ASSIGNED_TO_FORENSICS.name,
            DocumentStatus.UNDER_ANALYSIS.name,
            DocumentStatus.FORENSIC_VERIFIED.name,
        ...
        val userDocuments = MongoDB.documents.find(
            Filters.and(
                Filters.eq("userId", userId),
                Filters.`in`("status", validStatuses)
            )
        ).toList()
    ...
    private fun validateDocumentUpload(request: DocumentUploadRequest): List<ValidationError> {
        ...
        // Metadata validation
        if (!isLawEnforcementDoc(request.documentType)) {
    ...
    fun isLawEnforcementDoc(type: DocumentType): Boolean {
        return when (type) {
            DocumentType.FIR,
            ...
            DocumentType.LEGAL_NOTICE -> true
            else -> false
        }
    }
}
