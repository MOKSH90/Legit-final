...
fun Route.authRoutes(userService: UserService) {
    ...
    route("/api/v1/admin/auth") {
        authenticate("auth-admin") {
            post("/register-service-provider") {
                val request = call.receive<RegisterRequest>()
                val result = userService.registerServiceProvider(request)

                result.fold(
                    onSuccess = { authResponse ->
                        call.respond(
                            HttpStatusCode.Created,
                            ApiResponse.success(authResponse, "Service provider account created successfully")
                        )
                    },
                    onFailure = { error ->
                        when (error) {
                            ...
                            is ConflictException -> {
                                ...
                                call.respond(
                                    ...
                                    ApiError(
                                        code = ErrorCodes.INTERNAL_ERROR,
                                        message = error.message ?: "Service provider registration failed"
                                    )
                                )
                            }
                        }
    ...
    route("/api/v1/user") {
        authenticate("auth-jwt", "auth-session", strategy = AuthenticationStrategy.FirstSuccessful) {
            ...
            // ========================
            // PUT /api/v1/user/public-key
            // Register or update current user's asymmetric public key for E2EE
            // ========================
            put("/public-key") {
                val userId = extractUserId(call) ?: return@put
                val request = call.receive<UpdatePublicKeyRequest>()

                userService.updatePublicKey(userId, request.publicKey).fold(
                    onSuccess = {
                        call.respond(
                            HttpStatusCode.OK,
                            ApiResponse.ok("Public key updated successfully")
                        )
                    },
                    onFailure = { error ->
                        call.respond(
                            ...
                            ApiError(
                                code = ErrorCodes.INTERNAL_ERROR,
                                message = error.message ?: "Failed to update public key"
                            )
                        )
                    }
                )
            ...
            // ========================
            // GET /api/v1/user/{id}/public-key
            // Fetch public key for a specific user ID
            // ========================
            get("/{id}/public-key") {
                val targetUserId = call.parameters["id"]
                if (targetUserId.isNullOrBlank()) {
                    call.respond(HttpStatusCode.BadRequest, ApiError(code = ErrorCodes.BAD_REQUEST, message = "User ID is required"))
                    return@get
                }

                userService.getUserPublicKey(targetUserId).fold(
                    onSuccess = { keyResponse ->
                        call.respond(HttpStatusCode.OK, ApiResponse.success(keyResponse, "Public key retrieved"))
                    },
                    onFailure = { error ->
                        call.respond(HttpStatusCode.NotFound, ApiError(code = ErrorCodes.USER_NOT_FOUND, message = error.message ?: "User not found"))
                    }
                )
            }

            // ========================
            // GET /api/v1/user/directory/role/{role}
            // List public keys and users by role (e.g. FORENSICS_SPECIALIST, POLICE_OFFICER)
            // ========================
            get("/directory/role/{role}") {
                val roleStr = call.parameters["role"]?.uppercase()
                val role = try {
                    UserRole.valueOf(roleStr ?: "")
                } catch (e: Exception) {
                    call.respond(HttpStatusCode.BadRequest, ApiError(code = ErrorCodes.BAD_REQUEST, message = "Invalid role: $roleStr"))
                    return@get
                }

                val list = userService.getUsersByRole(role)
                call.respond(HttpStatusCode.OK, ApiResponse.success(list, "Users retrieved for role $roleStr"))
            }
