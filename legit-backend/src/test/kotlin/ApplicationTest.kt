...
class ApplicationTest {
    ...
    @Test
    fun testUnauthenticatedAdminRegisterServiceProvider() = testApplication {
        application {
            module()
        }
        client.post("/api/v1/admin/auth/register-service-provider") {
            contentType(ContentType.Application.Json)
            setBody("""{"username": "testbank", "email": "api@testbank.com", "password": "SecurePassword1!", "fullName": "Test Bank"}""")
        }.apply {
            assertEquals(HttpStatusCode.Forbidden, status)
        }
    }
